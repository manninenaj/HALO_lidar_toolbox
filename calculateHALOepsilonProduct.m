...
for DATEi = datenum(num2str(DATEstart),'yyyymmdd'):...
    ...
    if strcmp(windproduct,'winddbs')
        if not(isfield(wind_tday,'height'))
            wind_tday.height_agl = wind_tday.range(:).*sind(nanmedian(wind_tday.elevation(:)));
        end
    ...
    for ii = 1:length(fnames_var)
        ...
        if not(isempty(wind_tmrw) || isempty(wind_yday))
            ...
            if isfield(wind_tday,'wind_speed_error')
                ...
                ws_e_tmp = repmat(1.5,length(wstats.(fnames_time{ii})),length(wstats.height_agl)); % m s-1
            end
        else
            [Xo,Yo] = meshgrid(wind_tday.height_agl, wind_tday.time);
        ...
        beamdiameter = ones(length(wstats.(fnames_time{ii})),1) * transpose(wstats.height_agl) .* (C.beamwidth*pi/180);
        
        % Calculate length scales
        L_ii = ((3600.*dt_hrs.*wind) + 2 .* (ones(length(wstats.(fnames_time{ii})),1) * transpose(wstats.height_agl)) .* sin(beamdiameter./2));
        L1_ii = ((3600.*dt_raw.*wind) + 2 .* (ones(length(wstats.(fnames_time{ii})),1) * transpose(wstats.height_agl)) .* sin(beamdiameter./2));
    ...
    write_nc_struct(fullfile([dir_epsilon_out '/' thedate ...
