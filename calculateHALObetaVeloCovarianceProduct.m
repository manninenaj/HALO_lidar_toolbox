...
%     velo(wstats.signal_mean_3min<1.003) = nan; velo(:,1:3) = nan;
    ...
    for i = 1:length(dt)
        for j = 1:length(drange)
            ...
            lbob.confidence_interval_25(lbob.confidence_interval_25 == 0) = nan;
            lbob.confidence_interval_75(lbob.confidence_interval_75 == 0) = nan;
            ...
            cova_flux_conf_int_lo = reshape(lbob.confidence_interval_25,size(beta));
            cova_flux_conf_int_lo(cond_clean) = nan;
            cova_flux_conf_int_hi = reshape(lbob.confidence_interval_75,size(beta));
            ...
            data.(['covar_conf_25_' tres 'min_win_' winlen 'min_by_' winheight 'm']) = cova_flux_conf_int_lo;
            data.(['covar_conf_75_' tres 'min_win_' winlen 'min_by_' winheight 'm']) = cova_flux_conf_int_hi;
            data.(['nsamples_' tres 'min_win_' winlen 'min_by_' winheight 'm']) = nsamples;
            
            %%-- Create attributes --%%
            % cov
            att.(['covar_' tres 'min_window_' winlen 'min_by_' winheight 'm']) = create_attributes(...
                ...
                {[-0.1 0.1], 'linear'});
            % cov error
            att.(['covar_error_' tres 'min_window_' winlen 'min_by_' winheight 'm']) = create_attributes(...
                {['time_' tres 'min'],'height'},...
                ...
                {[-0.1 0.1], 'linear'});
            % cov conf int lo
            att.(['covar_conf_int_25_' tres 'min_window_' winlen 'min_by_' winheight 'm']) = create_attributes(...
                {['time_' tres 'min'],'height'},...
                'Lower confidence interval, 25th percentile of sample distribution',...
                {'Mm-1 sr-1 m s-1',''},...
                C.missing_value,...
                'Eq (1) in Engelmann et al., (2008).',...
                {[-0.1 0.1], 'linear'});
            % cov conf int hi
            att.(['covar_conf_int_hi_' tres 'min_window_' winlen 'min_by_' winheight 'm']) = create_attributes(...
                {['time_' tres 'min'],'height'},...
                'Higher confidence interval, 75th percentile of sample distribution',...
                {'Mm-1 sr-1 m s-1',''},...
                C.missing_value,...
                'Eq (1) in Engelmann et al., (2008).',...
                {[-0.1 0.1], 'linear'});
