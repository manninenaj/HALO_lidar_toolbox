...
for iDATE = datenum(num2str(DATEstart),'yyyymmdd'):...
    ...
    for idt1 = 1:length(dt)
        ...
        data.([bn '_instrumental_uncertainty_mean_' tres 'min']) = cell(numel(unique(ref.time_block_indices)),1);
        data.([bn '_instrumental_uncertainty_variance_' tres 'min']) = cell(numel(unique(ref.time_block_indices)),1);
        data.(['signal_instrumental_uncertainty_mean_' tres 'min']) = cell(numel(unique(ref.time_block_indices)),1);
        data.(['signal_instrumental_uncertainty_variance_' tres 'min']) = cell(numel(unique(ref.time_block_indices)),1);
    ...
    for ichunk = 1:numel(unique(ref.time_block_indices))
        ...
        for idt = 1:length(dt)
            ...
            X = []; % assign X as empty
            Y = ref.beta_raw(iv); % re-assign Y as beta
            Y_errors = ref.beta_raw(iv) .* ref.beta_error(iv); % assign Y_errors
            lbob.score_func = @weightedMean;
...
function [att] = addAttributes(C,tres,bn,weighting,att)
	    ...
	    'degrees');
        att.elevation.standard_name = 'Elevation from the horizon';        
        % Variables
        % nsamples
        ...
        att.([bn '_instrumental_uncertainty_mean_' tres 'min']) = create_attributes(...
            {['time_' tres 'min'],'range'},...
            ['Mean of the Doppler velocity instrumental uncertainty estimate(' tres ' min)'],...
            {'m s-1','m s<sup>-1</sup>'},...
            C.missing_value,...
        ...
        att.([bn '_instrumental_uncertainty_variance_' tres 'min']) = create_attributes(...
            {['time_' tres 'min'],'range'},...
            ['Variance of the Doppler velocity instrumental uncertainty estimate(' tres ' min)'],...
            {'m s-1','m s<sup>-1</sup>'},...
            C.missing_value,...
            'See Rye and Hardesty (1993). No correction for bias due to noise and sample size) correction.',...
            {[0 1], 'log'});
        ...
        att.(['signal_instrumental_uncertainty_mean_' tres 'min']) = create_attributes(...
            {['time_' tres 'min'],'range'},...
            ['Mean of fractional instrumental uncertainty in beta (' tres ' min)'],...
            {'m s-1','m s<sup>-1</sup>'},...
            C.missing_value,...
        ...
        att.(['signal_instrumental_uncertainty_variance_' tres 'min']) = create_attributes(...
            {['time_' tres 'min'],'range'},...
            ['Variance of fractional instrumental uncertainty in beta (' tres ' min)'],...
            {'m s-1','m s<sup>-1</sup>'},...
            C.missing_value,...
