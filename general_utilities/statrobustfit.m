...
if (nargout>1)
   ...
   if all(w<D | w>1-D)
       ...
       robust_s = my_statrobustsigma(wfun, radj, xrank, max(mad_s,tiny_s), tune, h);
   end
   ...
   [r,w,h,adjfactor] = insertNaNs(wasnan,r,w,h,adjfactor);
...
function [x1_o,x2_o,x3_o,x4_o] = insertNaNs(inan,x1_i,x2_i,x3_i,x4_i)
%insertNaNs inserts nans where they belong
% TBD: add input checks etc
%
% 2017-06-01
% Antti Manninen
...
if nargin > 5
    ...
    i_o = 1;
    i_i = 1;
    while i_i <= size(x1_i,1)
        switch inan(i_i)
            case 0 % do nothing, pass through
                x1_o(i_o) = x1_i(i_i);
                x2_o(i_o) = x2_i(i_i);
                x3_o(i_o) = x3_i(i_i);
                x4_o(i_o) = x4_i(i_i);
                i_i = i_i + 1;
                i_o = i_o + 1;
            case 1 % add nans in into the correct places
                x1_o(i_o) = nan;
                x2_o(i_o) = nan;
                x3_o(i_o) = nan;
                x4_o(i_o) = nan;
