...
%     nsamples = repmat(nsamples,size(y,1),1);
    switch method
        case 'unweighted'
            y_mean = nanmean(y);
            ...
            % sqrt of Eq.(28)
            y_stddev = sqrt(abs(M_2 - 1./nsamples .* nansum(y_error.^2)));
        case 'weighted'
            ...
            y_stddev = sqrt(abs(nansum(w.*(y-y_wmean).^2./repmat(V_1,size(y,1),1)) - (nsamples-1)./V_1));
    end
end
end
