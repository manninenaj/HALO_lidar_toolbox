...
%
%% Adapted from polarPcolor created by Etienne Cheynet
	...
	function createCircles(p)
        ...
        for kk=1:length(tickMesh)
            ...
            if abs(round(position))==90,
                % radial graduations
	        text((contourD(kk)).*cosd(110-p.thetaMin),...
		   (contourD(kk)).*sind(110-p.thetaMin),...
                    rtick,'verticalalignment','BaseLine',...
                    'horizontalAlignment', 'center',...
                    'handlevisibility','off','parent',cax);
                % annotate spokes
                text(contourD(end).*cosd(110-p.thetaMin),...
		     contourD(end).*sind(110-p.thetaMin),...
