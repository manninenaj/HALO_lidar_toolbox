...
%
% - bitflags            bitfield
%                       1st = constrained fit through 1 (not in use)
%                       2nd = 1st deg poly fit
%                       3rd = 2nd deg poly fit
%                       4th = correction failed, replaced with original
%                       5th = profile-by-profile correction failed, did nothing
%                       6th = profile-by-profile correction failed, replaced with nans
%                       7th = profile-by-profile correction failed, replaced with originals
%                       8th = profile-by-profile correction success
%                       9th = corrected with the averaged corrected background shape of the whole day
%
...
bit_1deg_fit = zeros(size(signal,1),1);
bit_2deg_fit = zeros(size(signal,1),1);
bit_correction_failed = zeros(size(signal,1),1);
bit_remnant_fit_failed_did_nothing = zeros(size(signal,1),1);
bit_remnant_fit_failed_replaced_w_nans = zeros(size(signal,1),1);
bit_remnant_fit_failed_replaced_w_originals = zeros(size(signal,1),1);
bit_remnant_fit_success = zeros(size(signal,1),1);
bit_final_fit_success = zeros(size(signal,1),1);
...
%                     ones(length(i_start_stp:i_end_stp), 1);
                bit_1deg_fit(i_start_stp:i_end_stp,1) = ...
                    ones(length(i_start_stp:i_end_stp), 1);
...
%                     repmat(2,length(i_start_stp:i_end_stp), 1);
                bit_2deg_fit(i_start_stp:i_end_stp,1) = ...
                    ones(length(i_start_stp:i_end_stp), 1);
                % Combine corrections to form corrected background
                background(i_start_stp:i_end_stp,:) = ...
                    (repmat(step_fitted(:),1,length(range_m)) - ...
...
%         flags = repmat(4,size(signal_0));
        bit_correction_failed = ones(size(signal_0));
        fprintf('failed.\n')
    end
...
%             % Estimate the mean shape in the background remaining
%             signal_SpinalTap = signal_remn;
...
%                 SpinalTap_fit = y_fit_1deg_SpinalTap;
%             else
%                 SpinalTap_fit = y_fit_2deg_SpinalTap;
%             end                
%             % Final corrected signal
%             signal_corr = signal_remn - transpose(repmat(SpinalTap_fit(:),1,size(signal_remn,1))) + 1;
...
%                 flags(remn_peak_loc) = 6;
                bit_remnant_fit_failed_replaced_w_nans(remn_peak_loc) = 1;
            end
...
%                 flags(remn_peak_loc) = 7;
                bit_remnant_fit_failed_replaced_w_originals(remn_peak_loc) = 1;
            end
    ...
    bitflags = bit_constrained_fit + ...
        2*bit_1deg_fit + 4*bit_2deg_fit + ...
        8*bit_correction_failed + ...
        16*bit_remnant_fit_failed_did_nothing + ...
        32*bit_remnant_fit_failed_replaced_w_nans + ...
        64*bit_remnant_fit_failed_replaced_w_originals + ...
        128*bit_remnant_fit_success + ...
        256*bit_final_fit_success;
...
%-- Subfunction - mySWT
    function [approx_coeff, detail_coeff] = mySWT(x, w_level, wavelet_type)
        ...
        switch wavelet_type
            ...
            case 'sym8'
                lopass = [-0.0034, -0.0005, 0.0317, 0.0076, -0.1433, -0.0613, 0.4814, 0.7772,  0.3644, -0.0519, -0.0272,  0.0491,  0.0038, -0.0150, -0.0003,  0.0019]; 
                hipass = [-0.0019, -0.0003, 0.0150, 0.0038, -0.0491, -0.0272, 0.0519, 0.3644, -0.7772,  0.4814,  0.0613, -0.1433, -0.0076,  0.0317,  0.0005, -0.0034]; 
            otherwise
