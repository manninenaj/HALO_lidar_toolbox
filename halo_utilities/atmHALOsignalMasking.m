function [cloud_mask,signal_cld_scrd] = atmHALOsignalMasking(signal,range,win_size,speedmode)
...
if nargin < 4
    speedmode = 'default';
elseif nargin == 4
    if ~ischar(speedmode) || all(~strcmp({'default','nocook','cookonly','doubleornothing'},speedmode))
        error('The 4th input ''speedmode'' must be a string and can be either ''default'', ''nocook'', ''cookonly'', or ''doubleornothing''.')
    end
end

switch speedmode
    case {'default','nocook','doubleornothing'}
        ...
        % allowed number of screened pixels is 5% of the total amount of
...
switch speedmode
    case {'default' ,'doubleornothing','cookonly'}
        ...
        signal_cld_scrd = signal_cld_scrd_outlr;
    case 'nocook'
        cloud_mask = isnan(signal_cloud_scr_var);
        signal_cld_scrd = signal_cloud_scr_var;
end
if strcmp(speedmode,'doubleornothing')
    for i_prof = 1:size(signal,1)
        if sum(~isnan(signal_cld_scrd_outlr(i_prof,:))) > ...
                numel(signal_cld_scrd_outlr(i_prof,:))*.05
            % Find outlier indices
